...
class WxxcxUser(http.Controller, BaseController):
    ...
    def login(self, sub_domain, code=None, **kwargs):
        try:
            ...
            config = request.env['wxxcx.config'].sudo()
            ...
            if not access_token:
                ...
                access_token = request.env(user=1)['wxxcx.access_token'].create({
                    'open_id': open_id,
    ...
    def register(self, sub_domain, code=None, encrypted_data=None, iv=None, **kwargs):
        '''
        用户注册
        '''
        try:
            ...
            request.env(user=1)['wxxcx.user'].create({
